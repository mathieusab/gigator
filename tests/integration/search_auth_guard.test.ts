...
// Integration test (in-process) for search route auth guard.
// Run with: npx tsx tests/integration/search_auth_guard.test.ts
...
async function buildFastifyWithRoutes() {
  ...
  const searchRoutes: (f: any) => Promise<void> = routesMod.default;
...
async function main() {
  ...
  {
    const res = await fastify.inject({ method: 'GET', url: '/api/search?q=x' });
  ...
  await fastify.close();
  console.log('PASS: search auth guard integration test passed');
  process.exit(0);
}

main().catch(async (err) => {
  console.error('FAIL: search auth guard integration test failed');
